 ...
 */
	...
	function link2($title, $url = null, $options = array(), $confirm = null) {
		if (!isset($url)) {
			$url = $title;
		}
		if (!isset($options['url'])) {
        ...
        if($options['confirm']!=null){
            $htmlOptions['onclick'] .= "cargar_contenido_pregunta('$url','$xup','".$options['confirm']."');";
        }else{
        	$htmlOptions['onclick'] .= "ver_documento('$url','$xup');";
        }
