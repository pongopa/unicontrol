 ...
 * @var array
 */
	public $helpers = array('Html', 'Ajax', 'Form', 'Session', 'Javascript');
 ...
 */
	public function input($fieldName, $options = array()) {
		...
		if (isset($options['url']) && isset($options['update'])) {
        	if($options['url']!='' && $options['update']!=''){
        		if(isset($options['onChange'])){
					$options['onChange'] .=" input_remoto('".$options['url']."','".$options['update']."',this.value);";
        		}else{
        			$options['onChange']="input_remoto('".$options['url']."','".$options['update']."',this.value);";
        		}
        	}
		...
		if (!isset($options['type'])) {
			...
			if (isset($type)) {
        		...
        		if (isset($type) && $type=='float') {
				    if(!isset($options['onBlur'])){
						$options['onKeyPress']="return numeros_con_punto(event);";
						$options['onBlur']="formato(this.id); limpia_campo(this.id,'0,00',2);";
						$options['style']=isset($options['style'])?$options['style']."text-align:right;":"text-align:right;";
						$options['onFocus'] = "limpia_campo(this.id,'0,00',1);";
				    }
        		}
				if(isset($options['numeric'])){
						$options['onKeyPress']="return numeros_con_punto(event);";
						$options['onBlur']="formato(this.id); limpia_campo(this.id,'0,00',2);";
						$options['style']=isset($options['style'])?$options['style']:"text-align:right;width:100%;";
						$options['onFocus'] = "limpia_campo(this.id,'0,00',1);";
				}
...
//				$options['value'] = $selected;
                if(isset($options['readonly'])){
                	if($options['readonly']==true){
                		$options['onmouseover'] = "this.disabled=true;";
                		$options['onmouseout']  = "this.disabled=false;";
                	}
                }
			...
			case 'date':
				$options['type']  = 'text';
				$options['id']    = 'fecha_'.$fieldName;
				$options['class'] = 'input_fecha';
				$options['readonly'] = !isset($options['readonly'])?false:$options['readonly'];
				$options['dateFormat'] = "D/M/Y";
				if($options['readonly']!=true){
					$options['after'] =  $this->script_input_fecha('fecha_'.$fieldName);
				}
 ...
 */
	...
	function input_fecha ($name,$id,$value=null) {
		echo "<input type='text' name='".$name."' id='".$id."' value='".$value."' class='input_fecha' readonly='true'>";
		echo "<script>//<![CDATA[
Calendar.setup({ trigger: \"$id\", inputField: \"$id\",dateFormat: \"%d/%m/%Y\" });
//]]></script>";
	}

	function script_input_fecha ($id=null) {
		echo "<script>//<![CDATA[
Calendar.setup({ trigger: \"$id\", inputField: \"$id\",dateFormat: \"%d/%m/%Y\" });
//]]></script>";
	}
	...
	function view_busqueda_index($model=null, $url=null, $update=null, $radio=array()){
		echo $this->Ajax->Form($model,'post',array('model'=>$model,'url'=>$url,'update'=>$update));
		echo'<table align="center" border="0">
			  <tr>
				<td width="70">Buscar:<br>';
				 echo $this->input('pista',array('label'=>false,'value'=>$_SESSION["BusquedaPista"], 'size'=>'26'));
		echo'   </td>';
		if($radio["activa"]==1){
			echo'   <td>';
				foreach($radio["datos"] as $k => $l){
					$options[strtoupper($k)] = strtoupper($l);
				}
			    echo $this->Form->input('radio',array('label'=>false,'type'=>'radio','legend'=>$radio["name"],'options'=>$options,'value'=>$_SESSION["BusquedaRadio"],'separator'=>'  '));
			echo'   </td>';
		}
		echo'   <td valign="BOTTOM">';
		         echo $this->submit('Filtrar',array('label'=>false));
		echo'   </td>';
		echo' </tr>
             </table>';
		echo $this->end();
