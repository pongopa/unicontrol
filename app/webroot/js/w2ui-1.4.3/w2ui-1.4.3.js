...
w2utils.keyboard = (function (obj) {
    ...
    function init() {
        ...
        jQuery(document).on('mousedown', mousedown);
    }
...
(function () {
    ...
    $.fn.w2overlay = function (html, options) {
        ...
        if ($('#w2ui-overlay'+ name).length > 0) {
            tmp_hide = $('#w2ui-overlay'+ name)[0].hide;
            jQuery(document).off('click', tmp_hide);
            if (typeof tmp_hide === 'function') tmp_hide();
        }
        $('body').append(
        ...
        setTimeout(function () {
            resize();
            jQuery(document).off('click', hide).on('click', hide);
            if (typeof options.onShow === 'function') options.onShow();
        }, 10);
        ...
        function resize () {
            ...
            if (div1.length > 0) {
                ...
                var maxHeight = window.innerHeight + jQuery(document).scrollTop() - div2.offset().top - 7;
                var maxWidth  = window.innerWidth + jQuery(document).scrollLeft() - div2.offset().left - 7;
                if ((maxHeight > -50 && maxHeight < 210) || options.openAbove === true) {
                    // show on top
                    maxHeight = div2.offset().top - jQuery(document).scrollTop() - 7;
                ...
                // check width
                w = div2.width();
                maxWidth = window.innerWidth + jQuery(document).scrollLeft() - div2.offset().left - 7;
        ...
        */
        ...
        if (menu === 'refresh') {
            ...
            $.fn.w2menuHandler = function (event, index) {
                ...
                // setTimeout(function () { jQuery(document).click(); }, 50);
            };
            var html = '';
            if (options.search) {
                html +=
    ...
    w2grid.prototype = {
        ...
        refreshRanges: function () {
            ...
            function mouseStart (event) {
                ...
                jQuery(document).off('mousemove', mouseMove).on('mousemove', mouseMove);
                jQuery(document).off('mouseup', mouseStop).on('mouseup', mouseStop);
            }
            ...
            function mouseStop (event) {
                ...
                jQuery(document).off('mouseup', mouseStop);
                // event after
        ...
        keydown: function (event) {
            ...
            switch (key) {
                ...
                case 91: // cmd key
                    ...
                    jQuery(document).on('keyup', tmp_key_down);
                    function tmp_key_down() {
                        $('#_tmp_copy_data').remove();
                        jQuery(document).off('keyup', tmp_key_down);
                    }
                    break;
        ...
        render: function (box) {
            ...
            function mouseStart (event) {
                ...
                jQuery(document).on('mouseup', mouseStop);
            }
                        ...
                        case 'w2ui-search-advanced':
                            ...
                            if (it.checked) {
                                ...
                                function tmp_close() {
                                    ...
                                    tb.uncheck(id);
                                    jQuery(document).off('click', 'body', tmp_close);
                                }
                                jQuery(document).on('click', 'body', tmp_close);
                            }
                            break;
                        case 'w2ui-add':
                            // events
            ...
            $(this.box).find('.w2ui-resizer')
                ...
                .on('mousedown', function (event) {
                    ...
                    var mouseUp = function (event) {
                        delete obj.resizing;
                        jQuery(document).off('mousemove', 'body');
                        jQuery(document).off('mouseup', 'body');
                        obj.resizeRecords();
                        // event before
                        obj.trigger($.extend(eventData, { phase: 'after', originalEvent: event }));
                    }
                    jQuery(document).on('mousemove', 'body', mouseMove);
                    jQuery(document).on('mouseup', 'body', mouseUp);
                })
    ...
    w2layout.prototype = {
        ...
        render: function (box) {
            ...
            function resizeStart(type, evnt) {
                ...
                if (!window.addEventListener) { window.document.attachEvent('onselectstart', function() { return false; } ); }
                jQuery(document).off('mousemove', obj.tmp.events.mouseMove).on('mousemove', obj.tmp.events.mouseMove);
                jQuery(document).off('mouseup', obj.tmp.events.mouseUp).on('mouseup', obj.tmp.events.mouseUp);
                obj.tmp.resize = {
                    type    : type,
                    x       : evnt.screenX,
                    y       : evnt.screenY,
            ...
            function resizeStop(evnt) {
                ...
                if (!evnt) evnt = window.event;
                if (!window.addEventListener) { window.document.attachEvent('onselectstart', function() { return false; } ); }
                jQuery(document).off('mousemove', obj.tmp.events.mouseMove);
                jQuery(document).off('mouseup', obj.tmp.events.mouseUp);
...
(function () {
    ...
    w2popup = {
        ...
        open: function (options) {
            ...
            // keyboard events
            if (options.keyboard) jQuery(document).on('keydown', this.keydown);
            ...
            function mvStart(evnt) {
                ...
                jQuery(document).on('mousemove', tmp.mvMove);
                jQuery(document).on('mouseup', tmp.mvStop);
            ...
            function mvStop(evnt) {
                ...
                tmp.resizing = false;
                jQuery(document).off('mousemove', tmp.mvMove);
                jQuery(document).off('mouseup', tmp.mvStop);
                w2popup.unlock();
            }
            return this;
        },
        ...
        close: function (options) {
            ...
            if (options.keyboard) jQuery(document).off('keydown', this.keydown);
        },
                ...
                if (it.type === 'drop' || it.type === 'menu') {
                    if (it.checked) {
                        ...
                        setTimeout(function () {
                            ...
                            jQuery(document).on('click', hideDrop);
                            function hideDrop() {
                                jQuery(document).off('click', hideDrop);
                                it.checked = false;
